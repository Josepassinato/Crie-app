...
const TrafficManagerPage: React.FC = () => {
    ...
    const {
        ...
        campaignPlan,
        analysisFeedback,
        isTrafficPlanLoading,
        isTrafficAnalysisLoading,
        error,
    ...
    const handlePlanInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        setTrafficPlanForm({ ...trafficPlanForm, [e.target.name]: e.target.value });
    };
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-brand-surface p-6 rounded-lg shadow-2xl border border-slate-700 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="productService" value={trafficPlanForm.productService} onChange={handlePlanInputChange} rows={3} placeholder={t('productServicePlaceholder')} className="w-full px-3 py-2 border border-slate-600 bg-slate-900 rounded-md text-brand-text"/>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-brand-subtle mb-2">{t('targetAudienceLabel')}</label>
                                <textarea name="targetAudience" value={trafficPlanForm.targetAudience} onChange={handlePlanInputChange} rows={3} placeholder={t('targetAudiencePlaceholder')} className="w-full px-3 py-2 border border-slate-600 bg-slate-900 rounded-md text-brand-text"/>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-brand-subtle mb-2">{t('mainObjectiveLabel')}</label>
                                <select name="objective" value={trafficPlanForm.objective} onChange={handlePlanInputChange} className="w-full px-3 py-2 border border-slate-600 bg-slate-900 rounded-md text-brand-text">
                            ...
                            <div>
                                ...
                                <input type="text" name="budget" value={trafficPlanForm.budget} onChange={handlePlanInputChange} placeholder={t('budgetPlaceholder')} className="w-full px-3 py-2 border border-slate-600 bg-slate-900 rounded-md text-brand-text"/>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-brand-subtle mb-2">{t('platformLabel')}</label>
                                <select name="platform" value={trafficPlanForm.platform} onChange={handlePlanInputChange} className="w-full px-3 py-2 border border-slate-600 bg-slate-900 rounded-md text-brand-text">
                            ...
                            <div>
                                ...
                                <input type="text" name="abTestRequest" value={trafficPlanForm.abTestRequest} onChange={handlePlanInputChange} placeholder={t('abTestPlaceholder')} className="w-full px-3 py-2 border border-slate-600 bg-slate-900 rounded-md text-brand-text"/>
                            </div>
                        </div>
                        <button onClick={handleCampaignPlanSubmit} disabled={isTrafficPlanLoading} className="w-full py-3 mt-4 text-lg font-medium text-white bg-gradient-to-r from-brand-primary to-brand-secondary rounded-md hover:opacity-90 disabled:bg-slate-700">
                            {isTrafficPlanLoading ? t('generatingPlan') : planButtonText}
                        </button>
                    </div>
                    ...
                    <div className="bg-brand-surface p-6 rounded-lg shadow-2xl border border-slate-700 space-y-6">
                            ...
                            <div className="mt-2 flex justify-center rounded-lg border border-dashed border-slate-600 px-6 py-10">
                                <div className="text-center">
                                    {trafficAnalysisImage ? (
                                        <img src={`data:${trafficAnalysisImage.mimeType};base64,${trafficAnalysisImage.base64}`} alt="Preview" className="mx-auto h-24 w-auto rounded-md" />
                                    ) : (
                        ...
                        <button onClick={handleCampaignPerformanceSubmit} disabled={isTrafficAnalysisLoading} className="w-full py-3 mt-4 text-lg font-medium text-white bg-gradient-to-r from-brand-primary to-brand-secondary rounded-md hover:opacity-90 disabled:bg-slate-700">
                            {isTrafficAnalysisLoading ? t('analyzingScreenshot') : analysisButtonText}
                        </button>
                    </div>
                          ...
                          onSelectHistoryItem={(item) => {
                             if (item.type === 'campaignPlan') {
                                ...
                                // setAnalysisFeedback(null);
                                 ...
                                 // setAnalysisFeedback(item.data as string);
